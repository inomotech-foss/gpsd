...
typedef enum {
    ...
    CASIC_TIM = 0x02,     // Timing message: time pulse output, time mark result
                ...
                const unsigned char *msg, size_t payload_len)
{
    ...
    if (0 != payload_len % 4) {
        GPSD_LOG(LOG_WARN, &session->context->errout,
                 "=> GPS: CASIC class: %02x, id: %02x, len: %zd NOT ALIGNED!\n",
                 msg_class, msg_id, payload_len);
    }
    ...
    unsigned checksum = casic_checksum((unsigned char *)session->msgbuf + 2, payload_len + 4);
                              ...
                              size_t payload_len UNUSED)
{
    ...
    GPSD_LOG(LOG_PROG, &session->context->errout,
             "CASIC: CFG-PRT: portID %d, protoMask %02x, mode %04x, baudRate %llu\n",
...
static gps_mask_t casic_parse(struct gps_device_t * session, unsigned char *buf,
                            size_t len)
{
    size_t payload_len;
    gps_mask_t mask = 0;
...
static void event_hook(struct gps_device_t *session, event_t event)
{
    ...
    /* We would like MON-VER
     * but it at least sometimes doesn't work. */
