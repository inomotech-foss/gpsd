...
. If you persistently fail to get live PPS, (a) you may have a
  ...
  problem, (e) you may have a device problem, (f) there may be a bug
...
The number "0.9999" is a placeholder as explained in the previous section about
configuring *ntpd*. It is *not a number to be used in production* - it's too
large. If you can't replace it with a real value, it would be best to not use
the SHM 0 source at all, keeping only the SHM 1 source.
