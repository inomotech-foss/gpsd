...
# platform might be necessary to use right build flags
target_platform = os.environ.get("TARGET_PLATFORM", sys.platform)
...
boolopts = (
    ...
    ("magic_hat", target_platform.startswith('linux'),
...
print("Target system is: %s" % target_platform)
...
if not cleaning and not helping:
    ...
    if ((config.CheckCompilerOption("-pthread") and
         not target_platform.startswith('darwin'))):
    ...
    if config.env['ncurses']:
        ...
        elif (target_platform.startswith('darwin') or
              target_platform.startswith('openbsd') or
              target_platform.startswith('sunos5')):
            ncurseslibs = ['-lcurses']
        else:
...
if ((not env['debug'] and not env['debug_opt'] and not env['profiling'] and
     not env['nostrip'] and not target_platform.startswith('darwin'))):
